...
impl<DB: AsClient> Viewable for View<DB> {
    ...
    fn view(&self) -> Element<'_, Message> {
        ...
        let main_view: iced::Element<_> = column![
            ...
            centered_row(errmsg),
            pub_list,
            centered_row(button("Enable subscription").on_press_maybe(
                if self.sub_curr.is_some() {
                    Some(Message::EnableSub)
                } else {
                    None
                }
            )),
    ...
    fn update(&mut self, msg: Message) -> Task<Message> {
        match msg {
            ...
            Message::EnableSub => {
                ...
                let enable_task = Task::perform(
                    Self::enable_sub(self.client.clone(), sub),
                    |val| match val {
                        Ok(()) => Message::Clear,
                        Err(err) => {
                            log::error!("{err}");
...
impl<DB: AsClient> View<DB> {
    ...
    async fn enable_sub(
        ...
        client
            .as_ref()
            .execute(&format!("ALTER SUBSCRIPTION \"{}\" ENABLE;", sub), &[])
            .await
            .map(|_| ())
    }
    ...
    async fn update_subs(
        ...
        let val = client
            .as_ref()
            .query(
                "SELECT subname FROM pg_subscription WHERE subenabled = 'f';",
                &[],
            )
            .await?;
